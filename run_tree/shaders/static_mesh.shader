...
float4 frag_main(Frag_Input input): SV_Target {
	float2 tex_coord = input.tex_coord * material_params.xy;
	...
	// float3 normal = sample_texture.Sample(normal_texture, normal_sampler, tex_coord) * 2 - 1;
	...
	float3 L = normalize(-directional_light_dir.xyz);
	...
	float diffuse_lighting = max(dot(normal, L), 0) * diffuse_intensity;
	float specular_lighting = pow(max(dot(normal, half_dir), 0.0), specular_shininess) * specular_intensity;
	float3 colour = diffuse*(ambient_lighting + diffuse_lighting + specular_lighting);
